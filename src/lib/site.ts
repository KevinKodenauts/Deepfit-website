export const SITE_URL = "https://deepfit.life";

export const SITE_EMAIL = "support@deepfit.life";

export const SITE_WHATSAPP = {
  display: "+971 54 751 2302",
  href: "https://wa.me/971547512302",
} as const;

export const SITE_COMPANY = {
  name: "Essential Brands Trading LLC",
  address: "Office 16, Creative Star Digital Solutions Bldg., IMPZ, Dubai, UAE",
} as const;

export const SITE_SOCIAL = [
  {
    label: "Instagram",
    href: "https://www.instagram.com/deepfitwellness?igsi=MWp3dHIyZzVrbXJhbg==",
  },
  {
    label: "Facebook",
    href: "https://www.facebook.com/share/1Br6Abq2aK/?mibextid=wwXIfr",
  },
] as const;
